...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="服务封面" v-if="type!='cross' || (type=='cross' && !ro.fuwufengmian)" prop="fuwufengmian">
            <file-upload
            tip="点击上传服务封面"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' class="upload" v-else label="服务封面" prop="fuwufengmian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.fuwufengmian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="费用" prop="feiyong">
            <el-input v-model.number="ruleForm.feiyong" 
                placeholder="费用" clearable :disabled=" false  ||ro.feiyong"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="适用人群" prop="shiyongrenqun">
            <el-input v-model="ruleForm.shiyongrenqun" 
                placeholder="适用人群" clearable :disabled=" false  ||ro.shiyongrenqun"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="服务区域" prop="fuwuquyu">
            <el-select v-model="ruleForm.fuwuquyu" placeholder="请选择服务区域" :disabled=" false  ||ro.fuwuquyu" >
              <el-option
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='fuwufengmian'){
              this.ruleForm.fuwufengmian = obj[o].split(",")[0];
        ...
        this.$http.get('option/fuwuquyu/fuwuquyu', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.fuwuquyuOptions = res.data.data;
          }
        });
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`yanglaofuwu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('yanglaofuwu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`yanglaofuwu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      fuwufengmianUploadChange(fileUrls) {
          this.ruleForm.fuwufengmian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
